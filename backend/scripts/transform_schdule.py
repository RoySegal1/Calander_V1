...
def transform_schedule(input_file, output_file, department):
    ...
    with open("scripts_consts.py", "r", encoding="utf-8") as file:
        lines = file.readlines()

    with open("scripts_consts.py", "w", encoding="utf-8") as file:
        for line in lines:
            if line.startswith("last_index"):
                file.write(f"last_index = {scripts_consts.last_index}\n")
            else:
                file.write(line)
...
def transform_schedule1(input_file, output_file, department):
    ...
    for entry in data:
        ...
        scripts_consts.last_index += 1
        unique_course_code = str(scripts_consts.last_index)
